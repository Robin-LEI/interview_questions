...
17. vuex不建议直接通过state去修改属性值，建议通过mutation间接修改

18. state相当于vue中的data，getters相当于vue中的计算属性computed

19. mutation中的方法不能直接调用，需要通过commit间接触发，而且通常在mutation中的做的都是同步操作

20. actions中的方法不能直接调用，需要通过dispatch触发，通常在actions中做一些异步操作，在actions中触发mutation中的方法修改属性值

21. new Vue会把data中的数据代理到vm实例上，但是对$开头的属性不会进行代理

22. vue中计算属性也会被放到vm实例上，计算属性具备缓存，不支持异步

23. vuex中计算属性不能修改

24. Vue.set可以动态设置不存在的属性为响应式

25. 所有模块的getters默认都会合并到一个对象上，除非你设置namespaced

26. commit时，传入的payload参数只能是一个基本值或者对象，也就是说只能传递一个

27. 既然有localstorage，为什么还需要`vuex-persist`数据持久化插件？答：localstorage存储的数据变了会改变视图吗？肯定不会

28. watch(oldValue, newValue)，在什么情况下，oldValue和newValue是一样的？答：当二者都是对象的时候
...
29. 格式化用户的配置
    ...
    class ModuleCollection {
      ...
      register(path, rootModule) {
        ...
        if (path.length == 0) {
          ...
          // []不走reduce 数组length为1，返回this.root
        ...
        if (rootModule.modules) {
          forEachValue(rootModule.modules, (value, key) => {
            // 递归调用
    ...
    let _module = new ModuleCollection(vuex);
    console.log(_module)
    ```
...
30. vuex的总体流程
    ...
    - 格式化用户传入的配置项，生成一个树
    - 根据这棵树进行模块安装，合并mutations、actions、state、getters模块
